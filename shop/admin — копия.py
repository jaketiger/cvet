...
class PostcardAdmin(SortableAdminMixin, ShopAdminStyleMixin, admin.ModelAdmin):
    ...
    # Убрали 'order' из list_editable, иначе не будет таскаться!
...
class BannerAdmin(SortableAdminMixin, ShopAdminStyleMixin, admin.ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        form.base_fields['image'].help_text = "Рекомендуется: 1200x400 (ПК). В режиме Адаптивно высота слайдера будет прыгать в зависимости от высоты каждой картинки (если они разные)."
        return form
...
class FooterPageAdmin(SortableAdminMixin, ShopAdminStyleMixin, admin.ModelAdmin):
    ...
    def special_page_warning(self, obj):
        if obj and obj.slug == 'contacts':
            return format_html(
                '<div style="background-color: #fff3cd; border: 1px solid #ffeeba; color: #856404; padding: 10px; border-radius: 5px;"><strong>ВНИМАНИЕ:</strong> Это страница "Контакты". <br>Данные берутся из раздела "Настройки сайта".</div>')
        elif obj and obj.slug in ['about', 'payment', 'terms']:
            return format_html('<span style="color: green;">✔ Используется специальный шаблон "{}"</span>', obj.slug)
        return "-"
