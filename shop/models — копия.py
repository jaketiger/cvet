...
FIT_MODE_CHOICES = [
    ('cover', 'Заполнить (Обрезать края)'),
    ('contain', 'Вписать целиком (С полями)'),
    ('auto', 'Адаптивно (По высоте картинки)'),
]
...
class SiteSettings(SingletonModel):
    ...
    processing_time = models.PositiveIntegerField("Время на сборку (мин)", default=50)
    close_cutoff = models.PositiveIntegerField("Блокировка 'Как можно быстрее' (мин)", default=20)
    ...
    working_hours = models.TextField("Режим работы (Текст)", blank=True)
...
class FooterPage(models.Model):
    ...
    slug = models.SlugField("URL-адрес (slug)", unique=True)
